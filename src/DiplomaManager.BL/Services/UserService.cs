            ...
            where T: User
        {
            ...
            var filters = new List<FilterExpression<User>>
            {
                new FilterExpression<User>(u =>
                    u is T &&
                    u.PeopleNames.Any(pn => pn.Name == firstName.Name) &&
                    u.PeopleNames.Any(pn => pn.Name == lastName.Name))
            };
            if (patronymic != null) //If Patronymic is missing
            {
                filters.Add(new FilterExpression<User>(u => u.PeopleNames.Any(pn => pn.Name == patronymic.Name)));
            }

            var user = Database.Users.Get(filters.ToArray()).SingleOrDefault();
